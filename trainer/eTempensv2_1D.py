...
#                          datetime.datetime.now().strftime("%Y-%m-%d-%H-%M"))
        self.save_dir  = '{}'.format(config.dataset)        
    ...
    def train_iteration(self, label_loader, unlab_loader, output_path, print_freq):
        ...
        for (label_x, label_y, ldx), (unlab_x, unlab_y, udx) in zip(cycle(label_loader), unlab_loader):
            ...
            unlab_outputs = unlab_outputs.to(self.device)
            iter_unlab_pslab = self.epoch_pslab[udx.long()]
            iter_unlab_pslab = iter_unlab_pslab.to(self.device)
            ...
            loop_info['lacc'].append(label_y.eq(outputs.max(1)[1].long()).double().sum().item()) 
            loop_info['uacc'].append(unlab_y.eq(unlab_outputs.max(1)[1].long()).double().sum().item())
            loop_info['lF1'].append(f1_score(label_y.cpu(), outputs.max(1)[1].long().cpu(), average='macro'))   
            loop_info['uF1'].append(f1_score(unlab_y.cpu(), unlab_outputs.max(1)[1].long().cpu(), average='macro'))
            if int(print_freq)>0 and (int(batch_idx)%int(print_freq))==0:
                print(f"[Semi train][{batch_idx:<3}]", self.gen_info(loop_info, lbs, ubs))
        # temporal ensemble
        self.update_ema_predictions() # update every epoch
        print(">>>[Semi train]", self.gen_info(loop_info, label_n, unlab_n, False))
        #New added print
        #output_path = 'C:/research/gitrepo/semisupervision_attn_cnn/results/USCHAD/'
        fp=open(os.path.join(output_path,'output.txt'),"a+")
        fp.write('Semi Train '+str(self.epoch)+': '+self.gen_info(loop_info, label_n, unlab_n, False)+'\n')
        fp.close()
    ...
    def test_iteration(self, data_loader, output_path, ep, num_classes, print_freq):
        ...
        for batch_idx, (data, targets) in enumerate(data_loader):
            ...
            loop_info['lF1'].append(f1_score(targets.cpu(), outputs.max(1)[1].cpu(), average='macro'))  
            if print_freq>0 and (batch_idx%print_freq)==0:
                print(f"[Semi test][{batch_idx:<3}]", self.gen_info(loop_info, lbs, ubs))
            outputsAll = np.concatenate((outputsAll, outputs.cpu()),axis=0)  
            
        pDoutputsAll = pd.DataFrame(outputsAll)
        pDoutputsAll.to_csv(os.path.join(output_path,'outputs'+str(ep)+'.csv'))
        print(">>>[Semi test]", self.gen_info(loop_info, label_n, unlab_n, False))   
        #New added print
        #output_path = 'C:/research/gitrepo/semisupervision_attn_cnn/results/USCHAD/'
        fp=open(os.path.join(output_path,'output.txt'),"a+")
        fp.write('Semi Test '+str(self.epoch)+': '+self.gen_info(loop_info, label_n, unlab_n, False)+'\n')
        fp.close()
    ...
    def validate_iteration(self, data_loader, output_path, ep, num_classes, print_freq):
        ...
        for batch_idx, (data, targets) in enumerate(data_loader):
            ...
            loop_info['lF1'].append(f1_score(targets.cpu(), outputs.max(1)[1].cpu(), average='macro'))  
            if print_freq>0 and (batch_idx%print_freq)==0:
                print(f"[semi val][{batch_idx:<3}]", self.gen_info(loop_info, lbs, ubs))
            outputsAll = np.concatenate((outputsAll, outputs.cpu()),axis=0)  
            
        pDoutputsAll = pd.DataFrame(outputsAll)
        pDoutputsAll.to_csv(os.path.join(output_path,'val_outputs'+str(ep)+'.csv'))
        print(">>>[semi val]", self.gen_info(loop_info, label_n, unlab_n, False))   
        #New added print
        #output_path = 'C:/research/gitrepo/semisupervision_attn_cnn/results/USCHAD/'
        fp=open(os.path.join(output_path,'output.txt'),"a+")
        fp.write('semi val '+str(self.epoch)+': '+self.gen_info(loop_info, label_n, unlab_n, False)+'\n')
        fp.close()
        return loop_info, label_n, outputsAll    
    ...
    def loop(self, epochs, output_path, scheduler, num_classes, label_data, unlab_data, unlab_data_nosh, test_data):
        ...
        self.epoch_pslab = self.create_soft_pslab(n_samples=len(unlab_data.dataset),
                                           n_classes=num_classes, dtype='rand')
        self.epoch_pslab = self.epoch_pslab.to(self.device)
        self.ema_pslab   = self.create_soft_pslab(n_samples=len(unlab_data.dataset),
                                           n_classes=num_classes, dtype='zero')
        self.ema_pslab = self.ema_pslab.to(self.device)
        ...
        #fp=open(os.path.join(output_path,'output.txt'),"a+")
        #fp.write('best '+': '+self.gen_info(best_info, n, n, False))        
        #fp.close()     
        return outputsAll
